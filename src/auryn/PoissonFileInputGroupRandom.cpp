...
#include "PoissonFileInputGroupRandom.h"
...
boost::mt19937 PoissonFileInputGroupRandom::gen = boost::mt19937(); 

void PoissonFileInputGroupRandom::init(AurynDouble  rate)
{
...
PoissonFileInputGroupRandom::PoissonFileInputGroupRandom( NeuronID n, AurynDouble rate) : SpikingGroup(n, RANKLOCK ) 
{
	present_pattern = false;
	init(rate);
}

PoissonFileInputGroupRandom::PoissonFileInputGroupRandom(
...
void PoissonFileInputGroupRandom::set_first_event(){
...
void PoissonFileInputGroupRandom::load_spikes(std::string filename)
{
...
void PoissonFileInputGroupRandom::load_events(std::string filename) {
...
void PoissonFileInputGroupRandom::sort_spikes(int type_pattern)
{
	std::sort (all_input_spikes[type_pattern].begin(), all_input_spikes[type_pattern].end(), time_compare);
}


AurynTime PoissonFileInputGroupRandom::get_offset_clock() 
{
	return sys->get_clock() - time_offset;
}

void PoissonFileInputGroupRandom::evolve()
{
...
// void PoissonFileInputGroupRandom::add_spike(double spiketime, NeuronID neuron_id)
// {
// 		SpikeEvent_type event;
...
void PoissonFileInputGroupRandom::set_rate(AurynDouble  rate)
{
...
AurynDouble  PoissonFileInputGroupRandom::get_rate()
{
	return lambda;
}


void PoissonFileInputGroupRandom::seed(unsigned int s)
{
	std::stringstream oss;
