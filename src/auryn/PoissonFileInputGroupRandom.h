...
#ifndef POISSONFILEINPUTGROUPRANDOM_H_
#define POISSONFILEINPUTGROUPRANDOM_H_
 ...
 *
 * PoissonFileInputGroupRandom first reads the entire ras file into memory and emits the spikes then during the simulation
 * without file access. It supports looping over the input spikes by setting the loop argument with the constructor 
 * to true.
 *
 */
class PoissonFileInputGroupRandom : public SpikingGroup
{
private:
...
public:
	...
	PoissonFileInputGroupRandom( NeuronID n, string filename_spikes , std::string filename_events, AurynDouble rate=5.,int nb_pattern =1);
	...
	PoissonFileInputGroupRandom( NeuronID n, AurynDouble rate=5.0);

	virtual ~PoissonFileInputGroupRandom();
	virtual void evolve();
...
#endif /*POISSONFILEINPUTGROUPRANDOM_H_*/
